...
- (void)updateOutlook
{
	switch (self.infoLabelStatus) {
		case InfoStatusFirstTimeSetting:
			self.infoLabel.text = @"设置模式！";
			break;
		case InfoStatusConfirmSetting:
			self.infoLabel.text = @"确认模式！";
			break;
		case InfoStatusFailedConfirm:
			self.infoLabel.text = @"无法确认，请重试";
			break;
		case InfoStatusNormal:
			self.infoLabel.text = @"绘制先前设定的模式去";
			break;
		case InfoStatusFailedMatch:
			self.infoLabel.text = [NSString stringWithFormat:@"猜错# %d，再试一次！",self.wrongGuessCount];
			break;
